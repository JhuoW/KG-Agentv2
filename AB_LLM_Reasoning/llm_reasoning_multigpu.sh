#!/bin/bash
# Multi-GPU LLM reasoning without Trie constraints
# Usage: bash AB_LLM_Reasoning/llm_reasoning_multigpu.sh
#
...
for DATA in ${DATA_LIST}; do
  for k in $K; do
    python AB_LLM_Reasoning/reasoning_llm_multigpu.py \
      --data_path ${DATA_PATH} \
      --d ${DATA} \
      --split ${SPLIT} \
